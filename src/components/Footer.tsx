...
export default function Footer({isAdmin, uploadFile, getFile}: Props) {
    ...
    return (
        <div className="flex items-center justify-center px-8 w-screen mt-36 relative aspect-[9/16] md:aspect-video">
            <img src="/footer.png" alt="footer" className="w-screen absolute bottom-0" />
            <div className="max-w-7xl w-screen flex flex-col-reverse gap-8 md:flex-row justify-between">
                {/* logos */}
                <div className="grid grid-cols-4 gap-2 items-end">
                    {
                        [1,2,3].map((_, index) => <FooterLogo uploadFile={uploadFile} getFile={getFile} isAdmin={isAdmin} index={index} />)
                    }
                </div>
                {
                    isAdmin ?
                    ...
                    : <p dir="rtl" className="bg-white bg-opacity-90 w-96 p-4 rounded-md aspect-video relative">{sampleText}</p>
                }
            </div>
        </div>
    )
